...
def binpack(articles,bin_cap):
   ...
   """
    ...
    fin = sorte[len(sorte)-1][0]
    while test == False:
        for i in range(len(sorte)-1):
            if load + sorte[i][1] <= bin_cap and sorte[i][2] == 0:
                hold.append(sorte[i][0])
                sorte[i][2] = 1
                load += sorte[i][1]
                i+=1
        if [sum(w) for w in zip(*sorte)][2] == len(sorte)-1:
            hold.append(fin)
            sorte[len(sorte)-1][2]= 1
            test = True
        bin_contents.append(hold)
        hold = []
        load = 0
